...
| `tools/scaffold/` | 案件にコピーされる雛形（CLAUDE.md / content.md） |
...
生成フォルダには `poster.html` / `poster_spec.md` / `content.md` / `CLAUDE.md` /
`tools/`（lint・render）/ `figures/` / `out/` が入り，**単体で lint・render が動く**．
以後の制作はそのフォルダ内の `CLAUDE.md` に従う（テンプレリポは触らない）．
...
- `lint`：font-size/color/border-radius/font-family の直書き，inline center，
  寸法の不一致（:root ↔ @page），24pt未満フォントを検出．エラー0で exit 0．
